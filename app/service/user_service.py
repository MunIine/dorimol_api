...
class UserService:
    def get_user(self, id_token: str) -> SUser:
        response = auth.verify_id_token(id_token=id_token, clock_skew_seconds=3) #TODO: пересмотреть

        uid = response["uid"]

        return SUser(
            uid=uid,
            role="user", #TODO: реализовать роли
            onboarding_required=False #TODO: реализовать онбординг
        )
